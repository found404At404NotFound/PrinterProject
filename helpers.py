...
def GENERATE_STATIC_ID(data:dict):
    """GENERATES A STATIC ID USING DATA DICTIONARY"""
    encrypt = Fernet(FERNET_KEY)
    encdata = encrypt.encrypt(json.dumps(data).encode('utf-8'))
    return encdata

def DECRYPT_STATIC_ID(enc_data: bytes):
    """DECRYPTS A STATIC ID TO DATA DICTIONARY"""
    if not (type(enc_data) == bytes):
        enc_data = enc_data.encode('utf-8')
    
    decrypt = Fernet(FERNET_KEY)
    dec_data = decrypt.decrypt(enc_data)
    dec_data = dec_data.decode('utf-8')
    dec_dt = json.loads(dec_data)
